class importerTool {
    constructor() {
        ...
        this.userInput = {
            ...
            formatMovies: null,                           // Detect movies and format as - Name (Year)
            formatSeries: null,                           // Detect series and format as - S01E01 Name
    ...
    async runFfmpeg() {
        ...
        for(const blob of blobs) {
            ...
            if(blob.type.startsWith('video/') || this.library.checkMimeTypeStr(blob.name)) {
                ...
                // Not a valid recording to reduce the number of recordings
                numRecs--;
            }
        }
    ...
    async createLibrary(blobs) {
        ...
        if(inputSuccess === true) {
            ...
            for(const blob of blobs) {
                ...
                const recordingData = await this.library.addRecording(blob, this.userInput, 0);
                
                // Get details from a data source if selected
                /*if(this.userInput.dataSourceOpt == 2) {
                    //iTunes.func(blobs);
                }*/
            ...
            /*
            if(userInput.formatSeries === true) {
            ...
            if(this.userInput.formatSeries === true) {
                for(const dir of this.library.directories) {
                    this.library.detectSeries(dir)
                }
            }
        }
    ...
    setUserInput() {
        const fullPath = this.validateFullPath($('#libraryDirectory').val());
        ...
        const formatMovies = $("#movie-filter").is(':checked');
        console.log(formatMovies);
        const formatSeries = $("#series-filter").is(':checked');
        ...
        this.userInput.formatMovies = formatMovies;
        this.userInput.formatSeries = formatSeries;
