    ...
    setFilePath(dirPath, status) {
        ...
        try {
            ...
            if(this.filePathArr[0]) {
                ...
                this.prevId = pathId;
                this.setFilePath(relativePath.slice(relativePath.indexOf(template.dirSeparator) + 1), status);
                ...
                if(!this.checkFileExists(this.prevId, relativePath)) {
                    ...
                    this.files[fileCount].status = status;
...
class recordingUI extends recordingsDirectoryStructure {
    ...
    createDirectoryStructure(dirPath, recNum, status) {
        debugMsg(`[info] Creating the directory structure for ${dirPath}`);
        try {
            this.recNum = recNum;
            this.setFilePath(template.rootDirName + template.dirSeparator + dirPath, status);
                    ...
                    <div class="panel-heading">
                        <h4 class="panel-title">
                            <span class="adjust-panel-heading">
                                ...
                                ${(this.files[recNum].status) ? ratingUi : statusUi}
                            </span>
                        </h4>
                    </div>
                </div>
