...
async function startRecordingFfmpeg(blobs) {
    ...
    // Disable buttons when adding recordings
    ...
    for (const blob of blobs) {
        ...
        const mimeType = template.checkMimeTypeStr(filename);
            ...
            // IF FAILD BLOB DOESN'T SUCCED ON RETRY THEN DONT WANT TO ADDrECORDING FOR IT
            if(!template.checkBlobExists(blob) && (blob.type.startsWith('video/') || mimeType !== false)) {
                ...
                template.setRecordingStatus(ffmpegStatus, template.recordingCount);
                recordingUi.createDirectoryStructure(relativePath, template.recordingCount, ffmpegStatus);
                template.recordingCount++;
            } else {
                //debugMsg("[info] Skipping file because it is not a video or it already exists");
